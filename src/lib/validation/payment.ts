...
export const paymentMethodValidation = {
  MPESA: z.object({
    txRef: z.string()
      ...
      .regex(/^[A-Z0-9]{10}$/, 'M-Pesa transaction code must be 10 characters (letters and numbers)'),
  }),
...
export function validatePaymentByMethod(payment: PaymentFormValues): { isValid: boolean; errors: string[] } {
  const errors: string[] = []
