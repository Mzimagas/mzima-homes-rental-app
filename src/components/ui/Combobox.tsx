...
export function Combobox<T extends ComboOption = ComboOption>({ options, value, onChange, label, placeholder, displayValue }: ComboboxProps<T>) {
  return (
    <div className="w-full">
      ...
      <HCombobox value={value || null} onChange={onChange as any}>
        <div className="relative mt-1">
          ...
          <Transition as={Fragment} leave="transition ease-in duration-100" leaveFrom="opacity-100" leaveTo="opacity-0" afterLeave={() => {}}>
            <HCombobox.Options className="absolute z-10 mt-1 max-h-60 w-full overflow-auto rounded-md bg-white py-1 text-base shadow-lg ring-1 ring-black/5 focus:outline-none sm:text-sm">
              {options.length === 0 ? (
                <div className="relative cursor-default select-none px-4 py-2 text-gray-700">No results</div>
              ) : (
                options.map((opt, idx) => (
                  <HCombobox.Option
                    key={opt.value}
