  ...
  COMMERCIAL_LAND: (props: React.SVGProps<SVGSVGElement>) => (
    <svg {...props} fill="none" stroke="currentColor" viewBox="0 0 24 24">
      <path
        ...
        strokeLinejoin="round"
        strokeWidth={2}
        d="M19 21V5a2 2 0 00-2-2H7a2 2 0 00-2 2v16l3.5-2 3.5 2 3.5-2 3.5 2zM10 8h4m-4 4h4"
      />
    </svg>
  ),
...
export default function PropertyTypeIcon({
  ...
  if (variant === 'background') {
    ...
    const backgroundClass =
      backgroundClasses[color as keyof typeof backgroundClasses] || backgroundClasses.gray
...
export function PropertyTypeIconSmall({
  type,
  className = '',
}: {
  type: PropertyType
...
export function PropertyTypeIconLarge({
  type,
  className = '',
}: {
  type: PropertyType
...
export function PropertyTypeIconEmoji({
  type,
  size = 'md',
  className = '',
}: {
...
export function PropertyTypeIconWithLabel({
  ...
  return (
    <div className={`inline-flex items-center ${spacingClasses[size]} ${className}`}>
      ...
      <span className={`${textSizeClasses[size]} font-medium`}>{label}</span>
    </div>
  )
}
