...
const ToastContext = React.createContext<{
  show: (message: string, opts?: { title?: string; variant?: ToastVariant; timeout?: number }) => void
} | null>(null)

export function ToastProvider({ children }: { children: React.ReactNode }) {
  const [toasts, setToasts] = React.useState<Toast[]>([])

  const show = React.useCallback((message: string, opts?: { title?: string; variant?: ToastVariant; timeout?: number }) => {
    const id = Math.random().toString(36).slice(2)
    const toast: Toast = { id, message, variant: opts?.variant || 'info', title: opts?.title, timeout: opts?.timeout ?? 4000 }
    setToasts((t) => [...t, toast])
