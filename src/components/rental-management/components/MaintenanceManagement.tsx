...
export default function MaintenanceManagement({
  ...
  const loadMaintenanceData = async () => {
    try {
      ...
      const { data: ticketsData, error: ticketsError } = await query.order('created_at', {
        ascending: false,
      })
      ...
      const totalEstimatedCost =
        ticketsArray.reduce((sum, t) => sum + (t.est_cost_kes || 0), 0) || 0
      const totalActualCost =
        ticketsArray.reduce((sum, t) => sum + (t.actual_cost_kes || 0), 0) || 0

      // Calculate average resolution time (simplified)
      const completedWithDates =
        ticketsArray.filter((t) => t.status === 'COMPLETED' && t.created_at && t.updated_at) || []
