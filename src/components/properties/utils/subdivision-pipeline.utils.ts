...
export const calculateSubdivisionOverallProgress = (stageData: SubdivisionPipelineStageData[]): number => {
...
export const getSubdivisionStageStatusColor = (stageData: SubdivisionPipelineStageData, isActive: boolean, isCompleted: boolean): string => {
...
export const getSubdivisionStageCardStyling = (isActive: boolean, isCompleted: boolean, canAccess: boolean): string => {
  if (isCompleted) {
...
export const getSubdivisionStageProgress = (stageData: SubdivisionPipelineStageData[]): { [key: number]: number } => {
...
export const mapSubdivisionStatusToStages = (subdivisionStatus: string): SubdivisionPipelineStageData[] => {
  ...
  return SUBDIVISION_PIPELINE_STAGES.map(stage => {
    if (stage.id < currentStageId) {
      ...
      const stageStartDate = new Date(baseDate.getTime() + (stage.id - 1) * 30 * 24 * 60 * 60 * 1000) // 30 days apart
      ...
      return {
        ...
        notes: stageNotes[stage.id as keyof typeof stageNotes] || `${stage.name} completed successfully`,
        documents: []
      }
    } else if (stage.id === currentStageId) {
      // Current stage is in progress
      const stageStartDate = new Date(baseDate.getTime() + (stage.id - 1) * 30 * 24 * 60 * 60 * 1000)
      ...
      return {
        ...
        status: 'In Progress',
        started_date: stageStartDate.toISOString(),
        notes: currentStageNotes[stage.id as keyof typeof currentStageNotes] || `Currently working on ${stage.name.toLowerCase()}`,
        documents: []
      }
    } else {
