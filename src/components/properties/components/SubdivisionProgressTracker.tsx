...
const getSubdivisionStageStatusColor = (stageData: SubdivisionPipelineStageData, isActive: boolean, isCompleted: boolean): string => {
  if (isCompleted) return 'bg-green-500 text-white'
  if (isActive) return 'bg-orange-500 text-white'
  return 'bg-gray-300 text-gray-600'
}

const getSubdivisionStageCardStyling = (isActive: boolean, isCompleted: boolean, canAccess: boolean): string => {
  if (isCompleted) {
  ...
  return (
    <div className="bg-white rounded-lg border border-gray-200 p-6">
          ...
          return (
            <div
              ...
              className={`relative p-4 rounded-lg border-2 cursor-pointer transition-all duration-200 transform hover:scale-105 ${
                getSubdivisionStageCardStyling(isActive, isCompleted, canAccess)
              }`}
            ...
            >
              ...
              <div className={`w-8 h-8 rounded-full flex items-center justify-center text-sm font-bold mb-2 ${
                getSubdivisionStageStatusColor(stageInfo!, isActive, isCompleted)
              }`}>
      ...
      <div className="mt-6 grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
        {stageData.filter(stage => stage.started_date || stage.completed_date).map((stage) => {
          ...
          return (
            <div key={stage.stage_id} className="bg-gray-50 rounded-lg p-3">
              ...
              <div className="text-xs text-gray-600 space-y-1">
                ...
                {stage.notes && (
                  <div className="text-gray-700 mt-1">{stage.notes}</div>
                )}
              </div>
            </div>
          )
