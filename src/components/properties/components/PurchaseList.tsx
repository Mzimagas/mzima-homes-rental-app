            ...
            <div className="grid grid-cols-1 md:grid-cols-3 gap-4 items-start mb-4">
              <div className="md:col-span-2">
                <div className="flex items-center space-x-3 mb-2">
                  <h3 className="text-lg font-semibold text-gray-900">{currentPurchase.property_name}</h3>
                  <span className="text-lg">{getSourceIcon('PURCHASE_PIPELINE')}</span>
                  <span className="text-xs bg-gray-100 text-gray-700 px-2 py-1 rounded-full">
                    {getSourceLabel('PURCHASE_PIPELINE')}
                  </span>
                  <span className={`text-xs px-2 py-1 rounded-full font-medium ${getPurchaseStatusColor(currentPurchase.purchase_status)}`}>
                ...
                <div className="flex flex-wrap gap-4 text-sm text-gray-500">
                  {currentPurchase.seller_name && <span>Seller: {currentPurchase.seller_name}</span>}
            ...
            <div className="flex items-center justify-between">
              <div className="flex space-x-2">
                ...
                <Button
                  variant="secondary"
                  size="sm"
                  onClick={() => setOpenDetailsId(openDetailsId === purchase.id ? null : purchase.id)}
                >
                  {openDetailsId === purchase.id ? 'Hide Details' : 'View Details'}
                </Button>

                {(currentPurchase.purchase_status === 'COMPLETED' || currentPurchase.overall_progress >= 100) && (
                  <Button
                    variant={currentPurchase.completed_property_id ? 'secondary' : 'primary'}
                    size="sm"
                    onClick={() => currentPurchase.completed_property_id ? (window.dispatchEvent(new CustomEvent('toast', { detail: { type: 'info', message: `Property already transferred: ${currentPurchase.property_name}` } }))) : onTransferProperty(currentPurchase)}
                    disabled={transferringId === purchase.id || !!currentPurchase.completed_property_id}
                  >
                    {transferringId === purchase.id ? (
                      ...
                      currentPurchase.completed_property_id ? 'Property Transferred' : 'Transfer to Properties'
                    )}
                  </Button>
                )}
              </div>
