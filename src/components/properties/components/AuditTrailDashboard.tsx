...
export default function AuditTrailDashboard({ purchaseId, userRole }: AuditTrailDashboardProps) {
  ...
  const handleApproval = async (approvalId: string, action: 'APPROVED' | 'REJECTED', notes?: string) => {
    try {
  ...
  const exportAuditLogs = () => {
    const csvContent = [
      ...
      ...auditLogs.map(log => [
        ...
        log.purchase_id,
        log.operation_type,
        Array.isArray(log.changed_fields) ? log.changed_fields.join(', ') : JSON.stringify(log.changed_fields),
        log.changed_by,
      ...
      .on('postgres_changes',
        ...
        (payload) => {
          ...
          if (payload.eventType === 'INSERT') {
            alert(`🔒 New audit log: ${payload.new.operation_type} operation on purchase ${payload.new.purchase_id}`)
          }
        }
      )
      .subscribe()
      ...
      <div className="p-6 border-b bg-gray-50">
        <div className="flex justify-between items-center mb-4">
          <div>
            ...
            <p className="text-sm text-gray-600">Monitor, analyze, and manage all security events</p>
          </div>
                        ...
                        <div key={field} className="bg-white rounded p-3 border">
                          ...
                          <div className="text-xs space-y-1">
                            <div className="text-red-600">
                              <span className="font-medium">Before:</span> {formatValue(log.old_values[field])}
                            </div>
                            <div className="text-green-600">
                              <span className="font-medium">After:</span> {formatValue(log.new_values[field])}
                            </div>
                          </div>
                        </div>
                      ))}
            ...
            <div className="space-y-6">
              ...
              <div className="grid grid-cols-1 md:grid-cols-4 gap-4">
                <div className="bg-blue-50 p-4 rounded-lg border border-blue-200">
                  <div className="text-2xl font-bold text-blue-800">{analytics.totalOperations}</div>
                ...
                <div className="bg-orange-50 p-4 rounded-lg border border-orange-200">
                  <div className="text-2xl font-bold text-orange-800">{analytics.requiresApprovalCount}</div>
                  <div className="text-sm text-orange-600">Required Approvals</div>
                </div>
                <div className="bg-purple-50 p-4 rounded-lg border border-purple-200">
                  <div className="text-2xl font-bold text-purple-800">
                    {Math.round((analytics.requiresApprovalCount / analytics.totalOperations) * 100)}%
                  </div>
              ...
              <div className="bg-gray-50 p-4 rounded-lg">
                ...
                <div className="space-y-2">
                  {Object.entries(analytics.operationsByDay)
                    .slice(-7)
                    .map(([day, count]) => (
                      <div key={day} className="flex items-center justify-between bg-white p-2 rounded">
              ...
              <div className="bg-yellow-50 p-4 rounded-lg border border-yellow-200">
                ...
                <div className="space-y-2 text-sm text-yellow-800">
                  <p>• {analytics.requiresApprovalCount} operations required approval in the last 30 days</p>
                  <p>• {analytics.uniqueUsers} unique users have made changes</p>
                  <p>• Most common operation: {Object.entries(analytics.operationsByType).sort(([,a], [,b]) => (b as number) - (a as number))[0]?.[0] || 'N/A'}</p>
