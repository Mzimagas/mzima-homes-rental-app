...
export default function DocumentList({ documents, onDocumentDeleted }: DocumentListProps) {
  ...
  const handleView = async (doc: Document) => {
    try {
      const { data } = await supabase.storage.from('documents').createSignedUrl(doc.file_path, 3600) // 1 hour expiry
            ...
            <div className="flex items-start space-x-4">
              ...
              <div className="flex-1 min-w-0">
                ...
                {doc.metadata?.description && (
                  <p className="text-sm text-gray-600 line-clamp-2">{doc.metadata.description}</p>
                )}
              </div>
            </div>
