...
import { UtilityBalanceService, UtilityAccountType } from '../../lib/services/utility-balance-service'

export default function UtilityAccountForm({ tenantId, unitId, onCreated }: { tenantId: string, unitId: string, onCreated?: () => void }) {
  ...
  return (
    <div className="space-y-3">
      <div>
        ...
        <select value={type} onChange={e => setType(e.target.value as UtilityAccountType)} className="border rounded p-2 w-full">
      ...
      {type === 'ELECTRICITY_PREPAID' && (
        <div>
          ...
          <input type="number" className="border rounded p-2 w-full" value={lowThreshold} onChange={e => setLowThreshold(e.target.value === '' ? '' : Number(e.target.value))} />
        </div>
      )}
      {type !== 'ELECTRICITY_PREPAID' && (
        <div>
          <label className="block text-sm">Credit Limit (KES)</label>
          <input type="number" className="border rounded p-2 w-full" value={creditLimit} onChange={e => setCreditLimit(e.target.value === '' ? '' : Number(e.target.value))} />
        </div>
      )}
