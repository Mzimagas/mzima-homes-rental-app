  ...
  const onSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (process.env.NEXT_PUBLIC_TURNSTILE_SITE_KEY && token) {
        ...
        if (!vRes.ok) { setError('Please complete the verification challenge and try again.'); setLoading(false); return }
      }

      const r = await fetch('/api/auth/resend', {
        method: 'POST', headers: { 'Content-Type': 'application/json', 'x-csrf-token': csrf }, body: JSON.stringify({ email })
      })
      if (!r.ok) { setError('Please try again later.'); setLoading(false); return }
      setOk(true)
    } catch {
    ...
    <div className="min-h-screen flex items-center justify-center bg-gray-50 py-12 px-4">
      <div className="max-w-md w-full space-y-8">
        ...
        <form onSubmit={onSubmit} className="space-y-4">
          ...
          <button disabled={loading} className="w-full bg-blue-600 text-white rounded px-4 py-2 disabled:opacity-50">{loading? 'Sending...' : 'Send'}</button>
        </form>
        <div className="text-center text-sm"><Link className="text-blue-600" href="/auth/login">Back to login</Link></div>
      </div>
    </div>
  )
}
