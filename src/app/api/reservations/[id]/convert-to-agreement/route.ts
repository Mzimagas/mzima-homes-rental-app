...
export async function POST(req: NextRequest, { params }: { params: { id: string } }) {
  try {
    ...
    const { data: resv, error: rErr } = await admin
      ...
      .select('*, units(unit_label, monthly_rent_kes, deposit_kes, property_id), properties:property_id(default_align_billing_to_start, default_billing_day)')
      .eq('id', params.id)
      .maybeSingle()
    ...
    if (resv.email) {
      const { data: t } = await admin.from('tenants').select('id').eq('email', resv.email).maybeSingle()
      tenantId = t?.id || null
    }
    if (!tenantId) {
      const { data: t2 } = await admin.from('tenants').select('id').eq('phone', resv.phone).maybeSingle()
      tenantId = t2?.id || null
    }
    if (!tenantId) {
      const { data: tNew, error: tErr } = await admin.from('tenants').insert({
    ...
    const { data: ag, error: agErr } = await admin.from('tenancy_agreements').insert({
      tenant_id: tenantId,
    ...
    await admin.from('reservation_requests').update({ status: 'APPROVED', handled_at: new Date().toISOString() }).eq('id', params.id)
