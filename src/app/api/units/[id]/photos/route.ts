...
async function checkUnitAccess(userId: string, unitId: string): Promise<boolean> {
  try {
    ...
    const { data: unit } = await admin.from('units').select('property_id').eq('id', unitId).maybeSingle()
    ...
    return membership?.status === 'ACTIVE' && ['OWNER', 'PROPERTY_MANAGER'].includes(membership.role)
  } catch {
    return false
  }
}
...
export async function POST(req: NextRequest, { params }: { params: { id: string } }) {
  try {
    ...
    // Get public URL
    const { data: { publicUrl } } = admin.storage.from('public-media').getPublicUrl(filePath)
