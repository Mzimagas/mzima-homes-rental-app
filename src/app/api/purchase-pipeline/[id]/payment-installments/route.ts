...
export const GET = compose(
  ...
  try {
    ...
    console.log('GET payment-installments - purchaseId:', purchaseId)

    // Check access
    const hasAccess = await checkPurchaseAccess(userId, purchaseId)
    if (!hasAccess) {
      console.log('GET payment-installments - hasAccess:', hasAccess)
...
export const POST = compose(
  ...
  try {
    ...
    console.log('POST payment-installments - purchaseId:', purchaseId)

    // Check access
    const hasAccess = await checkPurchaseAccess(userId, purchaseId)
    if (!hasAccess) {
      console.log('POST payment-installments - hasAccess:', hasAccess)
      return errors.forbidden()
    }
    ...
    console.error('Error in POST payment installments:', error)
    return errors.internal('Failed to create payment installment')
  }
})
