...
export const DELETE = compose(withAuth, withRateLimit, withCsrf)(async (req: NextRequest) => {
  try {
    ...
    console.log('DELETE payment-installment - paymentId:', paymentId)

    // Check access
    const hasAccess = await checkPurchaseAccess(userId, purchaseId)
    if (!hasAccess) {
      console.log('DELETE payment-installment - hasAccess:', hasAccess)
    ...
    console.error('Error in DELETE payment installment:', error)
    return errors.internal('Failed to delete payment installment')
  }
})
