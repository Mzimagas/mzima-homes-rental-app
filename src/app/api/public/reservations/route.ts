...
export async function POST(req: NextRequest) {
  try {
    ...
    const { unit_id, full_name, phone, email, preferred_move_in, message, turnstileToken } =
      parsed.data

    // Optional: Verify Turnstile token if provided
    if (turnstileToken) {
      const res = await fetch(
        `${process.env.NEXT_PUBLIC_APP_URL || ''}/api/security/turnstile-verify`,
        {
          method: 'POST',
    ...
    const { data: unit } = await supabase
      .from('units')
      .select('id, property_id, unit_label, monthly_rent_kes, deposit_kes')
      .eq('id', unit_id)
      .maybeSingle()
    ...
    try {
      ...
      if (serviceKey) {
        ...
        const activeStaff = (staff || []).filter(
          (r: any) => r.status === 'ACTIVE' && targetRoles.has(r.role)
        )
