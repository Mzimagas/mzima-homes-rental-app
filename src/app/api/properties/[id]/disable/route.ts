...
const Body = z.object({
  ...
  disableUnits: z.boolean().optional(),
})
...
async function handler(req: NextRequest) {
  ...
  const { data: unitsForProperty, error: unitsLoadErr } = await supabase
    .from('units')
    .select('id')
    .eq('property_id', propertyId)
  if (unitsLoadErr) {
      ...
      return NextResponse.json(
        {
          ok: false,
          code: 'ACTIVE_TENANCY',
          message: 'Property has active tenancies. End them before disabling.',
        },
        { status: 409 }
      )
    }
