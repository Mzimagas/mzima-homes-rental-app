...
export const DELETE = compose(
  ...
  try {
    ...
    const paymentId =
      paymentsIdx >= 0 && segments[paymentsIdx + 1] ? segments[paymentsIdx + 1] : undefined

    if (!propertyId || !paymentId)
      return errors.badRequest('Missing property id or payment id in path')
    ...
    if (!remainingError && remainingPayments) {
      ...
      for (const update of updates) {
        if (
          update.installment_number !==
          remainingPayments.find((p) => p.id === update.id)?.installment_number
        ) {
          await admin
            .from('property_payment_installments')
