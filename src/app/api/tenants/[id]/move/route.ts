...
export async function POST(req: NextRequest) {
  try {
    ...
    const { new_unit_id, monthly_rent_kes, move_date, reason, notes, end_current_agreement } =
      parse.data
    ...
    if (existingTenant) {
      return errors.badRequest(
        `Unit ${unit.unit_label} is currently occupied by ${existingTenant.full_name}`
      )
    }
    ...
    const alignBilling =
      parse.data.align_billing_to_start ?? prop?.default_align_billing_to_start ?? true
    ...
    const { data: createdAgreement, error: createErr } = await admin
      .from('tenancy_agreements')
      .insert({
        tenant_id: tenantId,
        unit_id: new_unit_id,
    ...
    const { error: prorationErr } = await admin.rpc('create_first_month_prorated_invoice', {
      p_tenancy_agreement_id: createdAgreement?.id,
    })
    if (prorationErr) {
