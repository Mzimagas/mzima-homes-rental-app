...
export default function NeighborhoodInfo({ address, propertyName }: NeighborhoodInfoProps) {
  const [activeTab, setActiveTab] = useState<'overview' | 'transport' | 'amenities' | 'schools'>('overview')

  // Mock data - in a real app, this would come from an API or database
  const neighborhoodData = {
    overview: {
      description: "This vibrant neighborhood offers a perfect blend of urban convenience and residential tranquility. Known for its tree-lined streets and friendly community atmosphere.",
      highlights: [
    ...
    transport: {
      publicTransport: [
        { type: "Bus Stop", name: "Main Street Bus Stop", distance: "2 min walk", routes: ["Route 14", "Route 23"] },
        { type: "Matatu Stage", name: "Junction Stage", distance: "5 min walk", routes: ["City Center", "Westlands"] }
      ],
      roads: [
        { name: "Main Highway Access", distance: "1.2 km", description: "Direct access to major highways" },
        { name: "Ring Road", distance: "800m", description: "Quick connection to other parts of the city" }
      ],
      walkScore: 75,
      bikeScore: 60
    },
          ...
          <div className="space-y-6">
                ...
                {neighborhoodData.transport.publicTransport.map((transport, index) => (
                  <div key={index} className="flex items-start space-x-3 p-3 bg-gray-50 rounded-lg">
                    ...
                    <div>
                      ...
                      <div className="text-xs text-gray-500">Routes: {transport.routes.join(', ')}</div>
                    </div>
                  </div>
                ))}
              </div>
            ...
            <div className="grid grid-cols-2 gap-4">
              <div className="text-center p-4 bg-blue-50 rounded-lg">
                <div className="text-2xl font-bold text-blue-600">{neighborhoodData.transport.walkScore}</div>
                <div className="text-sm text-gray-600">Walk Score</div>
              </div>
              <div className="text-center p-4 bg-green-50 rounded-lg">
                <div className="text-2xl font-bold text-green-600">{neighborhoodData.transport.bikeScore}</div>
                ...
                <div key={index} className="flex items-center justify-between p-4 bg-gray-50 rounded-lg">
                  ...
                  <div className="text-right">
                    <div className="flex items-center">
                      <span className="text-sm font-medium text-gray-900 mr-1">{school.rating}</span>
                      <div className="flex">
                        {[...Array(5)].map((_, i) => (
                          <svg
                            key={i}
