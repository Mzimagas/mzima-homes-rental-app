    ...
    const load = async () => {
      ...
      try {
        const { data, error } = await supabase
          ...
          .select(
            'id, unit_id, full_name, phone, email, preferred_move_in, status, created_at, units(unit_label)'
          )
          ...
          <div key={r.id} className="border rounded p-3 flex items-center justify-between">
            <div>
              <div className="font-medium">
                {r.full_name} — {r.phone} {r.email ? `• ${r.email}` : ''}
              </div>
              <div className="text-sm text-gray-600">
                Unit {r.units?.unit_label} • Preferred: {r.preferred_move_in || '—'} •{' '}
            ...
            <div className="flex gap-2 items-center">
              ...
              <button
                className="text-green-700 border px-2 py-1 rounded"
                onClick={() => updateStatus(r.id, 'APPROVED')}
              >
                Approve
              </button>
              <button
                className="text-yellow-700 border px-2 py-1 rounded"
                onClick={() => updateStatus(r.id, 'NEEDS_INFO')}
              >
                Need Info
              </button>
              <button
                className="text-red-700 border px-2 py-1 rounded"
                onClick={() => updateStatus(r.id, 'DECLINED')}
              >
                Decline
              </button>
              <button
                className="text-blue-700 border px-2 py-1 rounded"
                onClick={() => convertToAgreement(r.id)}
              >
