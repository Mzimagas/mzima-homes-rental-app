...
export default function EnhancedClientPropertyCard({
  ...
  const handleDownloadReport = () => {
    if (onDownloadReport) {
      onDownloadReport(property.id)
    }
  }

  // Get the best available location text
  const getLocationText = () => {
    // If we have coordinates, we can be more confident about the location
    if (property.lat && property.lng) {
      return property.physical_address || property.location || 'Precise location available'
    }
    
    // If we have address text, use it
    if (property.physical_address || property.location) {
      return property.physical_address || property.location
    }
  ...
  const isCompleted = property.handover_status === 'COMPLETED' || property.status === 'COMPLETED'
  ...
  return (
    <div className="bg-white rounded-lg shadow-sm border border-gray-200 overflow-hidden hover:shadow-md transition-shadow">
      ...
      <div className="p-4">
        ...
        {/* Current Stage for Active Properties */}
        {!showCompleted && property.current_stage && (
          <div className="mb-4 p-2 bg-blue-50 rounded-md">
            <p className="text-sm text-blue-800">
              <span className="font-medium">Current Stage:</span> {property.current_stage}
            </p>
          </div>
        )}
          ...
          {showCompleted ? (
            ...
            <>
              ...
              <button
                ...
                className="flex-1 bg-green-600 text-white px-4 py-2 rounded-md hover:bg-green-700 transition-colors font-medium flex items-center justify-center"
              >
                <span className="mr-1">📄</span>
                Download Report
              </button>
        ...
        {isClient && (
          <div className="mt-3 pt-3 border-t border-gray-200">
            <p className="text-xs text-gray-500 flex items-center">
              <span className="mr-1">🔒</span>
              {showCompleted ? 'Completed project • All documents available' : 'View-only access • Contact us for more information'}
            </p>
          </div>
        )}
      </div>

      {/* Expanded Read-Only Handover View */}
      {expanded && (
