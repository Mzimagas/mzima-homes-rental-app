...
export function useDashboardActions() {
  ...
  const updateTenantsCache = useCallback((tenants: Tenant[]) => {
    dispatch({ type: 'UPDATE_CACHE', payload: { type: 'tenants', data: tenants } })
  }, [dispatch])

  const updateUnitsCache = useCallback((units: Unit[]) => {
    dispatch({ type: 'UPDATE_CACHE', payload: { type: 'units', data: units } })
  }, [dispatch])

  const updatePaymentsCache = useCallback((payments: Payment[]) => {
    dispatch({ type: 'UPDATE_CACHE', payload: { type: 'payments', data: payments } })
  }, [dispatch])

  // Action tracking
  const addRecentAction = useCallback((action: Omit<Action, 'id' | 'timestamp'>) => {
  ...
  const isTenantSelected = useCallback((tenantId: string) => {
    return state.selectedTenant?.id === tenantId
  }, [state.selectedTenant])

  const isUnitSelected = useCallback((unitId: string) => {
  ...
  const getCachedData = useCallback((type: 'properties' | 'tenants' | 'units' | 'payments') => {
