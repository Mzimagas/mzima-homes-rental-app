...
test.describe('Payment Flow E2E Tests', () => {
  ...
  test('should complete full payment recording flow', async ({ page }) => {
    ...
    await expect(page.locator('[data-testid="payment-method-info"]')).toContainText(
      'Mobile money payment via M-Pesa'
    )
  ...
  test('should handle payment validation errors', async ({ page }) => {
    ...
    await page.click('[data-testid="next-step-button"]')
    await expect(page.locator('[data-testid="validation-error"]')).toContainText(
      'Maximum amount for M-Pesa is 300,000 KES'
    )
    ...
    await page.click('[data-testid="next-step-button"]')
    await expect(page.locator('[data-testid="validation-error"]')).toContainText(
      'M-Pesa transaction code must be 10 characters'
    )
  })
  ...
  test('should handle security warnings', async ({ page }) => {
    ...
    await expect(page.locator('[data-testid="security-warning-item"]')).toContainText(
  ...
  test('should handle payment notifications', async ({ page }) => {
    ...
    await expect(page.locator('[data-testid="notification-item"]').first()).not.toHaveClass(
      /unread/
    )
...
async function completePaymentForm(
  page: any,
  data: {
    tenant: string
    amount: string
