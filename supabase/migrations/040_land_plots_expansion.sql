-- 040: Property Type Integration - Expand system to support vacant land and plots
...
-- 8) Verify the migration worked by checking existing properties
DO $$
DECLARE
  property_count INTEGER;
BEGIN
  SELECT COUNT(*) INTO property_count FROM properties WHERE property_type IS NULL;
  IF property_count > 0 THEN
    RAISE EXCEPTION 'Migration failed: % properties have NULL property_type', property_count;
  END IF;

  RAISE NOTICE 'Migration successful: All properties have property_type set';
END $$;
