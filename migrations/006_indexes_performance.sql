-- Migration 006: Indexes and Performance Optimization
-- Creates additional indexes for optimal query performance
-- Note: CONCURRENTLY removed to work inside transaction blocks
...
CREATE INDEX CONCURRENTLY IF NOT EXISTS idx_parcels_acreage_range 
ON parcels(acreage_ha) 
WHERE acreage_ha IS NOT NULL;

CREATE INDEX CONCURRENTLY IF NOT EXISTS idx_plots_subdivision_stage 
ON plots(subdivision_id, stage);

CREATE INDEX CONCURRENTLY IF NOT EXISTS idx_plots_size_utility 
ON plots(size_sqm, utility_level) 
WHERE stage = 'ready_for_sale';

CREATE INDEX CONCURRENTLY IF NOT EXISTS idx_plots_premium_corner 
ON plots(premium_location, corner_plot) 
WHERE stage = 'ready_for_sale';

-- Client and sales indexes
CREATE INDEX CONCURRENTLY IF NOT EXISTS idx_clients_source_agent 
ON clients(source, agent_id);

CREATE INDEX CONCURRENTLY IF NOT EXISTS idx_clients_kyc_blacklist 
ON clients(kyc_verified, blacklisted);

CREATE INDEX CONCURRENTLY IF NOT EXISTS idx_listings_status_price 
ON listings(status, list_price) 
WHERE status = 'active';

CREATE INDEX CONCURRENTLY IF NOT EXISTS idx_offers_status_expiry 
ON offers_reservations(status, expiry_date);

CREATE INDEX CONCURRENTLY IF NOT EXISTS idx_sale_agreements_status_date 
ON sale_agreements(status, agreement_date);

-- Financial indexes
CREATE INDEX CONCURRENTLY IF NOT EXISTS idx_receipts_date_amount 
ON receipts(paid_date, amount) 
WHERE paid_date IS NOT NULL;

CREATE INDEX CONCURRENTLY IF NOT EXISTS idx_receipts_method_date 
...
CREATE INDEX CONCURRENTLY IF NOT EXISTS idx_commissions_agent_status 
ON commissions(agent_id, status);

CREATE INDEX CONCURRENTLY IF NOT EXISTS idx_commissions_payable_status 
ON commissions(payable_date, status) 
WHERE status = 'approved';

-- Document and audit indexes
CREATE INDEX CONCURRENTLY IF NOT EXISTS idx_documents_entity_type_current 
ON documents(entity_type, entity_id, doc_type) 
WHERE is_current_version = TRUE AND is_archived = FALSE;

CREATE INDEX CONCURRENTLY IF NOT EXISTS idx_documents_expiry_upcoming 
ON documents(expires_at) 
WHERE expires_at IS NOT NULL AND expires_at > CURRENT_DATE;

CREATE INDEX CONCURRENTLY IF NOT EXISTS idx_activities_audit_date_entity 
...
CREATE INDEX CONCURRENTLY IF NOT EXISTS idx_mpesa_transactions_date_processed 
ON mpesa_transactions(transaction_date, processed);

CREATE INDEX CONCURRENTLY IF NOT EXISTS idx_bank_mpesa_recons_date_status 
ON bank_mpesa_recons(transaction_date, status);

CREATE INDEX CONCURRENTLY IF NOT EXISTS idx_bank_mpesa_recons_unmatched 
...
CREATE INDEX CONCURRENTLY IF NOT EXISTS idx_tasks_reminder_date 
...
BEGIN
    ...
    IF EXISTS (SELECT 1 FROM pg_extension WHERE extname = 'postgis') THEN
        ...
        IF EXISTS (SELECT 1 FROM information_schema.columns WHERE table_name = 'parcels' AND column_name = 'geometry') THEN
            CREATE INDEX CONCURRENTLY IF NOT EXISTS idx_parcels_geometry
            ON parcels USING GIST(geometry)
            WHERE geometry IS NOT NULL;
            RAISE NOTICE 'Created spatial index for parcels';
        END IF;

        IF EXISTS (SELECT 1 FROM information_schema.columns WHERE table_name = 'plots' AND column_name = 'geometry') THEN
            CREATE INDEX CONCURRENTLY IF NOT EXISTS idx_plots_geometry
            ON plots USING GIST(geometry)
            WHERE geometry IS NOT NULL;
            RAISE NOTICE 'Created spatial index for plots';
        END IF;
    ELSE
...
CREATE INDEX CONCURRENTLY IF NOT EXISTS idx_parcels_lr_number_trgm 
ON parcels USING gin(lr_number gin_trgm_ops);

CREATE INDEX CONCURRENTLY IF NOT EXISTS idx_parcels_locality_trgm 
...
CREATE INDEX CONCURRENTLY IF NOT EXISTS idx_subdivisions_name_trgm 
ON subdivisions USING gin(name gin_trgm_ops);

-- Partial indexes for common filters
CREATE INDEX CONCURRENTLY IF NOT EXISTS idx_parcels_active_tenure 
ON parcels(tenure, county) 
WHERE current_use IS NOT NULL;

CREATE INDEX CONCURRENTLY IF NOT EXISTS idx_plots_available 
...
CREATE INDEX CONCURRENTLY IF NOT EXISTS idx_sale_agreements_active 
...
CREATE INDEX CONCURRENTLY IF NOT EXISTS idx_listings_price_per_sqm
...
CREATE INDEX CONCURRENTLY IF NOT EXISTS idx_sale_agreements_monthly 
ON sale_agreements(EXTRACT(YEAR FROM agreement_date), EXTRACT(MONTH FROM agreement_date), status);

CREATE INDEX CONCURRENTLY IF NOT EXISTS idx_plots_subdivision_summary 
ON plots(subdivision_id, stage) 
INCLUDE (size_sqm, utility_level, corner_plot, premium_location);

-- Unique constraints for business rules
CREATE UNIQUE INDEX CONCURRENTLY IF NOT EXISTS idx_unique_active_listing_per_plot 
ON listings(plot_id) 
WHERE status = 'active';

CREATE UNIQUE INDEX CONCURRENTLY IF NOT EXISTS idx_unique_active_offer_per_plot 
ON offers_reservations(plot_id) 
WHERE status IN ('reserved', 'accepted');

CREATE UNIQUE INDEX CONCURRENTLY IF NOT EXISTS idx_unique_active_agreement_per_plot 
ON sale_agreements(plot_id) 
WHERE status IN ('active', 'completed');

-- Indexes for foreign key constraints (if not automatically created)
CREATE INDEX CONCURRENTLY IF NOT EXISTS idx_parcel_owners_parcel_fk 
ON parcel_owners(parcel_id);

CREATE INDEX CONCURRENTLY IF NOT EXISTS idx_parcel_owners_owner_fk 
ON parcel_owners(owner_id);

CREATE INDEX CONCURRENTLY IF NOT EXISTS idx_encumbrances_parcel_fk 
ON encumbrances(parcel_id);

CREATE INDEX CONCURRENTLY IF NOT EXISTS idx_surveys_parcel_fk 
ON surveys(parcel_id);

CREATE INDEX CONCURRENTLY IF NOT EXISTS idx_subdivisions_parcel_fk 
ON subdivisions(parcel_id);

CREATE INDEX CONCURRENTLY IF NOT EXISTS idx_plots_subdivision_fk 
ON plots(subdivision_id);

-- Performance monitoring indexes
CREATE INDEX CONCURRENTLY IF NOT EXISTS idx_activities_audit_performance 
...
CREATE INDEX CONCURRENTLY IF NOT EXISTS idx_user_activities_cleanup 
ON user_activities(created_at) 
WHERE created_at < CURRENT_DATE - INTERVAL '1 year';

CREATE INDEX CONCURRENTLY IF NOT EXISTS idx_data_access_logs_cleanup 
