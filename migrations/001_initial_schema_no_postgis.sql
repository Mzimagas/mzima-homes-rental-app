-- Migration 001: Initial Schema - Core Entities (No PostGIS)
-- Creates core tables for land management system without PostGIS dependencies
...
BEGIN
    ...
    -- Create user_role if it doesn't exist, or update it if it does
    IF NOT EXISTS (SELECT 1 FROM pg_type WHERE typname = 'user_role') THEN
        CREATE TYPE user_role AS ENUM ('super_admin', 'admin', 'manager', 'sales_agent', 'finance', 'operations', 'viewer');
    ELSE
        -- Check if 'viewer' exists in the enum, if not add it
...
CREATE INDEX IF NOT EXISTS idx_parcels_lr_number ON parcels(lr_number);
...
CREATE INDEX IF NOT EXISTS idx_parcels_tenure ON parcels(tenure);
CREATE INDEX IF NOT EXISTS idx_parcels_current_use ON parcels(current_use);
CREATE INDEX IF NOT EXISTS idx_parcels_acquisition_date ON parcels(acquisition_date);

CREATE INDEX IF NOT EXISTS idx_owners_id_number ON owners(id_number);
CREATE INDEX IF NOT EXISTS idx_owners_kra_pin ON owners(kra_pin);
CREATE INDEX IF NOT EXISTS idx_owners_entity_type ON owners(entity_type);
CREATE INDEX IF NOT EXISTS idx_owners_full_name_gin ON owners USING gin(full_name gin_trgm_ops);

CREATE INDEX IF NOT EXISTS idx_parcel_owners_parcel_id ON parcel_owners(parcel_id);
CREATE INDEX IF NOT EXISTS idx_parcel_owners_owner_id ON parcel_owners(owner_id);
CREATE INDEX IF NOT EXISTS idx_parcel_owners_active ON parcel_owners(is_active);

CREATE INDEX IF NOT EXISTS idx_encumbrances_parcel_id ON encumbrances(parcel_id);
CREATE INDEX IF NOT EXISTS idx_encumbrances_type ON encumbrances(encumbrance_type);
...
CREATE INDEX IF NOT EXISTS idx_activities_audit_entity ON activities_audit(entity_type, entity_id);
CREATE INDEX IF NOT EXISTS idx_activities_audit_actor ON activities_audit(actor_id);
CREATE INDEX IF NOT EXISTS idx_activities_audit_created_at ON activities_audit(created_at);
...
BEGIN
    ...
    DROP TRIGGER IF EXISTS update_encumbrances_updated_at ON encumbrances;
