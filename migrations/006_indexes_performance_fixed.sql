-- Migration 006: Indexes and Performance Optimization (Fixed)
-- Creates additional indexes for optimal query performance
-- CONCURRENTLY removed to work inside transaction blocks
...
-- Overdue invoices index (removed CURRENT_DATE predicate due to immutability)
...
-- Documents expiry index (removed CURRENT_DATE predicate due to immutability)
...
-- Recent receipts index (removed CURRENT_DATE predicate due to immutability)
...
-- Index on the price_per_sqm column
...
-- Performance monitoring indexes (removed CURRENT_DATE predicates due to immutability)
...
-- Maintenance indexes for cleanup operations (removed CURRENT_DATE predicates due to immutability)
