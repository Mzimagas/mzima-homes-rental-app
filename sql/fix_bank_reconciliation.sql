-- Bank Reconciliation Tables Fix
...
DROP VIEW IF EXISTS reconciliation_analytics;

-- Reconciliation analytics view
CREATE VIEW reconciliation_analytics AS
SELECT 
    ...
    ROUND(
        CASE 
            WHEN COUNT(bt.id) > 0 THEN 
                (COUNT(CASE WHEN bt.status = 'MATCHED' THEN 1 END)::DECIMAL / COUNT(bt.id)) * 100
            ELSE 0 
        END, 2
...
-- Drop existing view if it exists
DROP VIEW IF EXISTS unmatched_transactions;
...
INSERT INTO bank_accounts (account_name, account_number, bank_name, bank_code, account_type, is_primary, current_balance_kes, opening_balance_kes) VALUES
('Mzima Homes Main Account', '1234567890', 'Equity Bank', 'EQBLKENA', 'CURRENT', true, 2500000, 2000000),
('Mzima Homes M-PESA', '254700000000', 'Safaricom M-PESA', 'MPESA', 'MPESA', false, 350000, 300000),
('Mzima Homes Savings', '0987654321', 'KCB Bank', 'KCBLKENX', 'SAVINGS', false, 5000000, 4500000)
...
DECLARE
    ...
    mpesa_account_id UUID;
    kcb_account_id UUID;
BEGIN
    -- Get account IDs
    SELECT id INTO equity_account_id FROM bank_accounts WHERE account_number = '1234567890';
    SELECT id INTO mpesa_account_id FROM bank_accounts WHERE account_number = '254700000000';
    SELECT id INTO kcb_account_id FROM bank_accounts WHERE account_number = '0987654321';
    
    -- Insert sample transactions for Equity Bank
    IF equity_account_id IS NOT NULL THEN
        INSERT INTO bank_transactions (
            bank_account_id, transaction_date, transaction_ref, description, 
            amount_kes, transaction_type, source, status
        ) VALUES
        (equity_account_id, CURRENT_DATE - INTERVAL '5 days', 'EQ001', 'Rental Income - Property A', 45000, 'CREDIT', 'BANK_STATEMENT', 'MATCHED'),
        (equity_account_id, CURRENT_DATE - INTERVAL '3 days', 'EQ002', 'Maintenance Payment - Plumber', -15000, 'DEBIT', 'BANK_STATEMENT', 'UNMATCHED'),
        (equity_account_id, CURRENT_DATE - INTERVAL '2 days', 'EQ003', 'Member Contribution - John Doe', 25000, 'CREDIT', 'BANK_STATEMENT', 'UNMATCHED'),
        (equity_account_id, CURRENT_DATE - INTERVAL '1 day', 'EQ004', 'Utility Bill Payment', -8500, 'DEBIT', 'BANK_STATEMENT', 'MATCHED')
        ON CONFLICT (bank_account_id, transaction_ref, transaction_date) DO NOTHING;
    END IF;
    
    -- Insert sample transactions for M-PESA
    IF mpesa_account_id IS NOT NULL THEN
        INSERT INTO bank_transactions (
            bank_account_id, transaction_date, transaction_ref, description, 
            amount_kes, transaction_type, source, status
        ) VALUES
        (mpesa_account_id, CURRENT_DATE - INTERVAL '4 days', 'MP001', 'M-PESA Rental Payment', 30000, 'CREDIT', 'MPESA_STATEMENT', 'UNMATCHED'),
        (mpesa_account_id, CURRENT_DATE - INTERVAL '2 days', 'MP002', 'Emergency Repair Payment', -12000, 'DEBIT', 'MPESA_STATEMENT', 'UNMATCHED'),
        (mpesa_account_id, CURRENT_DATE, 'MP003', 'Member Fee Payment', 5000, 'CREDIT', 'MPESA_STATEMENT', 'UNMATCHED')
        ON CONFLICT (bank_account_id, transaction_ref, transaction_date) DO NOTHING;
    END IF;
    
    -- Insert sample transactions for KCB
    IF kcb_account_id IS NOT NULL THEN
        INSERT INTO bank_transactions (
            bank_account_id, transaction_date, transaction_ref, description, 
            amount_kes, transaction_type, source, status
        ) VALUES
        (kcb_account_id, CURRENT_DATE - INTERVAL '7 days', 'KCB001', 'Property Sale Proceeds', 500000, 'CREDIT', 'BANK_STATEMENT', 'MATCHED'),
        (kcb_account_id, CURRENT_DATE - INTERVAL '1 day', 'KCB002', 'Interest Earned', 2500, 'CREDIT', 'BANK_STATEMENT', 'MATCHED')
        ON CONFLICT (bank_account_id, transaction_ref, transaction_date) DO NOTHING;
    END IF;
END $$;
