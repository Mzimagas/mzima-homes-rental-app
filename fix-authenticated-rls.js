...
async function fixAuthenticatedRLS() {
  console.log('🔧 Fixing RLS policies for authenticated role...');
  console.log('The issue: Infinite recursion exists for authenticated users but not service role');
  
  try {
    // Step 1: Completely disable RLS temporarily to clear all policies
    ...
    // We'll execute each statement separately since exec_sql doesn't exist
    const statements = disableSQL.split(';').filter(s => s.trim());
    for (const stmt of statements) {
      if (stmt.trim()) {
        try {
          // Try to execute via a simple query approach
    ...
    // Step 3: Create completely new, simple, non-recursive policies
    ...
    const createPoliciesSQL = `
      ...
      -- PROPERTIES: Direct landlord access (no subqueries)
    ...
    // For now, let's test if we can at least create the policies via direct SQL execution
    console.log('\n5. Testing the fix...');
    
    // Test with anon key (same as browser)
    const testClient = createClient(
      process.env.NEXT_PUBLIC_SUPABASE_URL,
      process.env.NEXT_PUBLIC_SUPABASE_ANON_KEY
    );
    
    const { data: testProps, error: testError } = await testClient
      .from('properties')
      .select('id, name')
      .limit(1);
    
    if (testError) {
      console.error('❌ Test still fails:');
      console.dir(testError, { depth: null });
      
      if (testError.message && testError.message.includes('infinite recursion')) {
        console.log('🚨 Infinite recursion still exists - need to apply SQL manually');
      ...
      console.log('✅ Test successful! RLS policies fixed.');
    }
