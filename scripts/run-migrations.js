...
async function ensureMigrationsTable(supabase) {
  ...
  try {
    ...
    const { error: queryError } = await supabase.from('schema_migrations').select('id').limit(1)
      ...
      try {
        ...
        if (error) {
          ...
          if (statement.toUpperCase().includes('CREATE TABLE')) {
            logInfo(
              `Skipping CREATE TABLE statement (may already exist): ${statement.substring(0, 50)}...`
            )
            continue
          }
