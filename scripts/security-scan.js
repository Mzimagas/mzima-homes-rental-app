...
const SECURITY_PATTERNS = [
  ...
  { pattern: /postgres:\/\/[^:\s]+:[^@\s]+@[^:\s]+:\d+\/\w+/, type: 'PostgreSQL Connection String', severity: 'HIGH' },
  { pattern: /mongodb:\/\/[^:\s]+:[^@\s]+@[^:\s]+:\d+\/\w+/, type: 'MongoDB Connection String', severity: 'HIGH' },
  ...
  { pattern: /\b[0-9a-f]{8}-[0-9a-f]{4}-[0-9a-f]{4}-[0-9a-f]{4}-[0-9a-f]{12}\b/i, type: 'UUID', severity: 'LOW' },
...
function generateReport(findings) {
  ...
  log(`   Medium severity: ${bySeverity.MEDIUM.length}`, bySeverity.MEDIUM.length > 0 ? 'yellow' : 'green')
